...
| Email/report sending | `index.html`, `netlify/functions/send-workorder.js` | `sendEmailNow()` generates the PDF, posts it to `send-workorder`, and the Netlify Function sends through Resend. On success, `markWorkOrderEmailed()` merge-patches `lastEmailedAt`/`lastEmailedTo` onto the `workorders` doc, surfaced as "📧 Emailed …" in the Saved tab; `logReportAndHistoryEvent()` separately logs the durable report/timeline entry (now including `emailSubject`). See "Visible email-sent record" in `DEV_NOTES.md`. |
