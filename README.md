...
| Local storage quota safety | `js/core.js` | `saveDb()` keeps the `localStorage` cache under a 3MB budget (of a ~5MB browser quota) by walking a three-rung eviction ladder — full-resolution bytes, then thumbs, then whole cached records, oldest first — proactively on every write, and again if the quota is hit anyway. Never touches the open order, an unsynced one, a photo whose bytes exist nowhere else, or a record that never reached the cloud. See "Storage is full on Report Preview" and "Local work order cache" in `DEV_NOTES.md`. |
